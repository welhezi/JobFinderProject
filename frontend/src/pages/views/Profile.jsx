...
import { updateEmployeeAction } from '../../redux/actions/employeeAction';

const Profile = () => {
  const { currentUser } = useSelector(state => state.employee); // 🔹 utiliser employee slice
  ...
  useEffect(() => {
    if (!user._id) return; // éviter erreur si user vide
    setFormData({
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        if (formData[key] !== null) updatedData.append(key, formData[key]);
      });
      updatedData.append('id', user._id);

      const resultAction = await dispatch(updateEmployeeAction(updatedData)).unwrap();
  ...
  // 🔹 Ne pas afficher la page si ce n'est pas un employee
  if (!user.role || user.role !== 'employee') {
    return (
      <Box sx={{ minHeight: '100vh', py: 6, px: { xs: 2, sm: 4, md: 6 } }}>
      ...
      <Grid container spacing={4} justifyContent="center">
        ...
        <Grid item xs={12} md={3}>
          <Card sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: 3, boxShadow: '0 8px 25px rgba(0,0,0,0.08)', backgroundColor: '#ffffff', p: 4, height: '100%', width: '300px', minHeight: '350px' }}>
            ...
            <Typography variant="body2" color="text.secondary" mb={2}>{user.role === 'employee' ? 'Employee' : 'User'}</Typography>
        ...
        <Grid item xs={12} md={4}>
          <Card sx={{ borderRadius: 3, boxShadow: '0 8px 25px rgba(0,0,0,0.08)', backgroundColor: '#ffffff', p: 2, height: '100%', minHeight: '350px' }}>
            <CardContent>
              ...
              <Divider sx={{ mb: 3 }} />
              <Box mb={2} display="flex" alignItems="center"><BusinessIcon sx={{ color: '#dd2571', mr: 1 }} /><Typography variant="body1">{user.company || 'Not provided'}</Typography></Box>
              <Box mb={2}><Typography variant="body2" color="text.secondary">Description:</Typography><Typography variant="body1">{user.description || '—'}</Typography></Box>
      ...
      <CustomModel title="Edit Profile" open={openEdit} handleClose={() => setOpenEdit(false)} icon={<EditIcon />}>
        <form onSubmit={handleUpdate} encType="multipart/form-data">
          <Stack spacing={2}>
            ...
            <TextField label="Company" name="company" value={formData.company} onChange={e => setFormData({ ...formData, company: e.target.value })} fullWidth />
            <TextField label="Description" name="description" value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} multiline rows={3} fullWidth />
